    ...
    <div id="container">
        ...
        <div class="clearfix">
            ...
            <div class="content_l">           
	                <div class="c_detail">
	                	<div style="background-color:#fff;" class="c_logo">
		                	<a title="上传公司LOGO" id="logoShow" class="inline cboxElement" href="#logoUploader">
		                				                			<img width="190" height="190" alt="公司logo" src="company_logo/<?=$arr["comp_logo"]?>">
	                    ...
	                    <div class="c_box companyName">
	                        	...
	                        	<a target="_blank" class="applyC" href="http://www.lagou.com/c/auth.html">申请认证</a>
	                        	                        <div class="clear"></div>
	                       		                   			...
	                       		                   			<h1  class="fullname"><?=$arr["comp_name"]?></h1>
	                        	                        
	                        <form class="clear editDetail dn" id="editDetailForm" style="display: none;">
	                            <input type="text" placeholder="请输入公司简称" maxlength="15"  name="companyShortName" id="companyShortName" class="valid">
	                           <!-- <input type="text" placeholder="一句话描述公司优势，核心价值" maxlength="50"  name="companyFeatures" id="companyFeatures" class="valid"><span for="companyFeatures" generated="true" class="error" style="display: none;">请输入5-50字的一句话介绍</span>-->
	                        ...
	                        <div class="clear oneword" style="display: block;"><img width="17" height="15" src="style/images/quote_l.png">&nbsp; <span><?=$arr["comp_abb"]?></span> &nbsp;<img width="17" height="15" src="style/images/quote_r.png"></div>
				...
				<input type="hidden" id="companyid" value="<?=$arr["id"]?>">
       				...
       				<div id="Profile">
					            ...
					            <!--编辑介绍-->
					                <dl class="c_section newIntro dn" style="display: none;" id="self1">
					                    <dt>
					                        <h2><em></em>公司介绍</h2>
					                    </dt>
					                    <dd>
						                    <form id="companyDesForm">
						                        ...
						                        <div class="clear"></div>
						                        <input type="submit" value="保存" id="sub1" class="btn_small">
					            ...
					            <!--有介绍-->
					               <dl class="c_section" style="display: block;" id="self2">
					               		<dt>
					                   		<h2><em></em>公司介绍</h2>
					                   	</dt>
					                   	<dd>
					                   		<div class="c_intro" id="selfcontent"><?=$arr["comp_self"]?></div>
					                   		<a title="编辑公司介绍" id="editIntro" class="c_edit" href="javascript:void(0)"></a>
					                   	</dd>
					               	</dl>
<script>
$(function(){
	//$(document).delegate("#subm1","click",function(){
	$("#sub1").click(function(){
		var text=$("#companyProfile").val();
		var id=$("#companyid").val();
		var updself='?r=company/updatecompanyself';
		//alert(id);
		$.ajax({
			type:'post',
			url:updself,
            	...
            	<div id="Tags">
	            	<div id="c_tags_show" class="c_tags solveWrap" style="display: block;">
	                    <table><tbody>
							<tr>
								<td>地点</td>
								<td id="city_a"><?=$arr["comp_city"]?></td>
							</tr>
							<tr>
								<td>领域</td>
								<td><?=$arr["industry_sector"]?></td>
							</tr>
							<tr>
								<td>规模</td>
								<td id="scale_a"><?=$arr["comp_scale"]?></td>
							</tr>
							<tr>
								<td>主页</td>
								<td><a target="_blank" href="#" id="url_a"><?=$arr["comp_url"]?></a></td>
							</tr>
							</tbody></table>
		                ...
		                <form id="tagForms">
		                    <table>
		                        <tbody><tr>
		                            <td>地点</td>
		                            <td>
		                            	<input type="text" placeholder="请输入地点" value="<?=$arr["comp_city"]?>" name="city" id="city" class="valid">
		                            </td>
		                        </tr>
		                        <tr>
		                            <td>领域</td><!-- 支持多选 -->
		                            <td>
		                            	...
		                            	<input type="button" style="background:none;cursor:default;border:none !important;" disable="disable" value="<?=$arr["industry_sector"]?>" id="select_ind" class="select_tags">
		                        ...
		                        <tr>
		                            ...
		                            <td>
		                            	...
		                            	<input type="button" value="<?=$arr["comp_scale"]?>" id="select_sca" class="select_tags">
		                        ...
		                        <tr>
		                            <td>主页</td>
		                            <td>
                            			<input type="text" placeholder="请输入网址" value="<?=$arr["comp_url"]?>" name="companyUrl" id="companyUrl" class="valid">
		                            </td>
		                        </tr>
		                    </tbody></table>
		                    ...
		                    <input type="submit" value="保存" id="sub3" class="btn_small">
...
<script>
$(document).delegate("#sub3","click",function(){
	var city=$("#city").val();
	var scale=$("#select_sca").val();
	var url=$("#companyUrl").val();
	var id=$("#companyid").val();
	var updcsu='?r=company/updatecompanycsu';
	//alert(url)
	$.ajax({
		type:'post',
		url:updcsu,
		...
		success:function(data)
		{
			...
			else if(data==3)
			{
				...
				<dl class="c_section c_stages">
                	<dt>
                    	<h2><em></em>融资阶段</h2>
                    	<a title="编辑融资阶段" class="c_edit" href="javascript:void(0)" id="sta_a"></a>
                    </dt>
                    <dd>
                    	<ul class="reset stageshow" id="stage1">
                    		<li>目前阶段：<span class="c5" id="sp1"><?=$arr["comp_stage"]?></span></li>
                    	</ul>
                    	<form class="dn" id="stageform">
                    		<div class="stageSelect">
                    			<label>选择阶段</label>
                    			<!--<input type="hidden" value="<?/*=$arr["comp_stage"]*/?>" id="financeStage" name="financeStage">-->
	                         	<input type="button" value="<?=$arr["comp_stage"]?>" id="select_fin" class="select_tags_short fl">
	                    	...
	                    	<!--<ul id="stagesList" class="reset">
								<li>
				                    ...
				                    <input type="button" value="请选择" class="select_tags_short select_invest">
		                    ...
		                    <input type="submit" value="保存" class="btn_small" id="sub2">
		                    <a id="cancelStages" class="btn_cancel_s" href="javascript:void(0)">取消</a>
		                    <div class="clear"></div>
<script>
$(function(){
//	$(document).delegate("#sub2","click",function(){
	$("#sub2").click(function(){
		var stage=$("#select_fin").val();
		var id=$("#companyid").val();
		var updstage='?r=company/updatecompanystage';
		$.ajax({
			type:'post',
			url:updstage,
			data:{
		                ...
		                <dl class="c_section c_member">
		                    ...
		                    <dd> 
		                    	<div class="member_wrap">
									<!-- 无创始人 -->
									<div class="member_info addnew_right dn">展示公司的领导班子，<br>提升诱人指数！<br><a class="member_edit" href="javascript:void(0)">+添加成员</a>
									 </div>
				                      <!-- 编辑创始人 -->
									<div class="member_info newMember dn" id="ori_b">
										<form class="memberForm">
											<div class="new_portrait">
												...
												<div class="portraitShow">
													<img width="120" height="120" src="<?=$arr["ori_pic"]?>">
													<span>更换头像</span>
												</div>
						                        ...
						                        <input type="text" placeholder="请输入创始人姓名" value="<?=$arr["originator"]?>" name="name" class="valid" id="ori_name">
					                            <input type="text" placeholder="请输入创始人当前职位" value="<?=$arr["ori_pos"]?>" name="position" id="ori_pos">
					                            <input type="text" placeholder="请输入创始人新浪微博地址" value="<?=$arr["ori_sina"]?>" name="weibo" class="valid" id="ori_sina">
					                            <textarea placeholder="请输入创始人个人简介" maxlength="500" class="s_textarea valid" name="remark" id="ori_self"><?=$arr["ori_self"]?></textarea>
					                            <div class="word_count fr">你还可以输入 <span>476</span> 字</div>
					                            <div class="clear"></div>
					                            <input type="submit" value="保存" class="btn_small" id="sub4">
				                        ...
				                        <!-- 显示创始人 -->
				                    	<div class="member_info" id="ori_a">
	                    					<a title="编辑创始人" class="c_edit member_edit" href="javascript:void(0)"></a>
				                        	<div class="m_portrait">
				                            	<div></div>
				                            	<img width="120" height="120" alt="创始人" src="<?=$arr["ori_pic"]?>">
					                        </div>
				                            <div class="m_name"><span id="or_sp"><?=$arr["originator"]?></span><a target="_blank" class="weibo" href="<?=$arr["ori_sina"]?>" id="or_sina"></a></div>
				                            <div class="m_position" id="or_pos"><?=$arr["ori_pos"]?></div>
				                    		<div class="m_intro" id="or_self"><?=$arr["ori_self"]?></div>
				                        </div>
...
<script>
$(document).delegate("#sub4","click",function(){
	var originator=$("#ori_name").val();
	var ori_pos=$("#ori_pos").val();
	var ori_sina=$("#ori_sina").val();
	var ori_self=$("#ori_self").val();
	var id=$("#companyid").val();
	var updori='?r=company/updateori';
	$.ajax({
		type:'post',
		url:updori,
		data:{
		...
		success:function(data){
			...
			if(data==1)
			{
				$("#ori_b").css('display','none');
				$("#ori_a").css('display','block');
			//	$("#sta_a").css('display','block');
				$("#or_sp").html(originator);
				$("#or_self").html(ori_self);
				$("#or_pos").html(ori_pos);
				$("#or_sina").attr('href',ori_sina);
				//alert(data)
			}
			else if(data==2)
			{
