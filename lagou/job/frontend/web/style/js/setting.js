$(function(){
	...
	$(function(){
		$('#pwd').keyup(function(){
			...
			if($('#ppd').html()=="<font color='red'>密码错误</font>")
			{
				$('#ppd').html("<font></font>");
				return;
			}
		...
		$('#pwd').blur(function(){
			...
			//alert(password);
			$.post(path,{password:password,'_csrf':csrf},function(e){
				if(e==0){
					$('#ppd').html("<font color='red'>密码错误</font>");
				}else{
					$('#ppd').html("<font></font>");
				}
	...
	$('#updatePswForm').validate({
    	...
    	/*submitHandler:function(form){
    		...
    		var resubmitToken = $('#resubmitToken').val();
    		$.ajax({
    			url:"index.php?r=personal/updatepwd",
    			type:'POST',
    			data:{
    				'_csrf':csrf,
    				Password:password,
